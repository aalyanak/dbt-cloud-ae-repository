    ...
    create or replace table `analytics-engineering-398318`.`ae_tier2_curated`.`user_conversion`
...
live_session AS (
    ...
    union all
    select * from `analytics-engineering-398318`.`ae_tier1_raw`.`user_conversion_organic_live_session`
),

wo_live_session AS (
    select * from `analytics-engineering-398318`.`ae_tier1_raw`.`user_conversion_without_live_session`
),
